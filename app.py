...
def home_page():
    ...
    if 'user' in session:
        ...
        home_content = db.session.query(FiliaUser, PhotoPost).join(PhotoPost, FiliaUser.user_id == PhotoPost.user_id).all()

        posts = []
        for user, post in home_content:
            if user.major == user_major:
                post_data = (user, post)  # Include only user and post in the post data
                posts.append(post_data)  # Append a tuple of user and post to the posts list
...
def unfollow(user_id):
    ...
    db.session.commit()
    redirect(f'/profile_page/{user.user_id}')
...
def follow(user_id):
    ...
    db.session.commit()
    redirect(f'/profile_page/{user.user_id}')#python file is flask, and html file is jinja


@app.route('/profile_page', endpoint='profile_page_without_id')
@app.route('/profile_page/<int:user_id>', endpoint='profile_page_with_id')
def profile_page(user_id=None):
    if user_id is None:
        user_id = session['user']['user_id']

    user = FiliaUser.query.get(user_id)
    is_user_page = user_id == session['user']['user_id']
    is_following = False
    if not is_user_page:
    ...
    return render_template('profile_page.html', profile_posts=profile_posts, is_following=is_following, num_following=num_following, 
                           num_followers=num_followers, user_id=user_id, user=user, num_posts=num_posts, is_user_page=is_user_page)
...
def delete_photo_post(post_id):
    ...
    photo_path = os.path.join(app.config['UPLOAD_FOLDER_POSTS'], os.path.basename(post.photo_path))
...
@app.get('/uploadprofpic')
def uploadprofpic():
    return render_template('editprofpic.html')
...
def like_post(post_id):
    ...
    if user in post.likes:
        ...
        post.likes.append(user)  # Like the post
...
def deleteprofpic():
    ...
    profile_path = os.path.join(app.config['UPLOAD_FOLDER_PROFILE'], os.path.basename(user.profile_path))
